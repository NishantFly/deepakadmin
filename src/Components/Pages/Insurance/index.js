...
function Vehicle(props) {
  ...
  useEffect(() => {
    ...
    let url = "http://localhost:2999/api/getinsurance/by/admin";
    setisloading(true);
    axios
      .get(url,config)
      .then(
        (res) => {
          console.log("data viewInsurance:::", res);
          setisloading(false);
          setInsuranceDataArry(res.data.getinsurancebyAdmin);
      ...
      <div className="home_padding">    
        <div className="content_padding">
          <Grid className="Component_main_grid mb-3">
            <Grid item md={9}>
              <h3 className="mb-2">Insurance</h3>
              <button
                type="button"
                class="btn btn-info mr-4"
                onClick={() => props.history.push("/add/insurance")}
              >
              ...
              <Table className={classes.table} aria-label="simple table">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>New Insurance</TableCell>
                ...
                <TableBody>
                  {(rowsPerPage > 0
                    ...
                    <TableRow key={row.name}>
                      ...
                      <TableCell>{row.addinsurance?.insurancedate}</TableCell>
                      <TableCell>{row.addinsurance?.insurancedocname}</TableCell>
                      <TableCell>{row.newinsurance}</TableCell>
